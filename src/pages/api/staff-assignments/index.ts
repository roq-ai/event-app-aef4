...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getStaffAssignments() {
    ...
    const response = await prisma.staff_assignment
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'staff_assignment'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createStaffAssignment() {
    ...
    const data = await prisma.staff_assignment.create({
      data: body,
    });
