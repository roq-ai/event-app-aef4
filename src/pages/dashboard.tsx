...
import { EventListPage as ShowCaseList } from 'pages/events';

function DashboardPage() {
  const { session } = useSession();

  const registrationStat = useSWR<PaginatedInterface<RegistrationInterface>>(
    () => `/registrations`,
    () => getRegistrations({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Registrations',
      result: registrationStat?.data?.totalCount,
      isLoading: registrationStat.isLoading,
